...
class OuraClient {
    ...
    enum Endpoints {
        ...
        var stringValue: String {
            ...
            case .fetchSleepData(let startDate, let endDate):
                ...
                //return "http://172.16.98.57:3000/sleepData?start_date=\(startDate)&end_date=\(endDate)"
            }
        }
    ...
    class func fetchSleepData(completion: @escaping (OuraSleepResponse?, Error?) -> Void) {
        taskForGETRequest(url: Endpoints.fetchSleepData("2021-04-05", ActivityDateConverter.getStringFromDate(date: Date())).url, response: OuraSleepResponse.self, completion: completion)
    }
