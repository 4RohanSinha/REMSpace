...
struct SleepLogGraph: View {
    ...
    init(sleepLogs: [SleepLogEntry], currentSleepLogIdx: Int?) {
            ...
            VStack {
                GeometryReader { geometry in
                    ZStack {
                        ...
                        SleepLogGraphDataPoints(dataPoints: sleepLogs, highlightEntry: sleepLogs[currentSleepLogIdx ?? (sleepLogs.count - 1)], rectDimensions: (width: geometry.size.width, height: geometry.size.height))
